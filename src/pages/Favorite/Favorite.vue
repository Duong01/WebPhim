<template>
  <v-container class="favorite-page" fluid>
    ...
    <div v-else-if="movies.length === 0" class="empty-state">
      ...
      <v-btn color="red" class="mt-4" to="/home">
        {{ $t("Khám phá phim") }}
      </v-btn>
    </div>
...
<v-row v-if="recentMovies.length" class="mb-4">
  <v-col cols="12">
          ...
          <v-row no-gutters>
            ...
            <v-col cols="4" class="poster-col">
              <div class="poster-wrapper">
                ...
                <div
                  class="tag-new"
                  v-if="getNextEpisode(movie).includes('Tập')"
                >
                  Sắp ra
                </div>
                <div class="tag-new-comp" v-else>Hoàn tất</div>
              </div>
            </v-col>
            ...
            <v-col cols="8" class="content-col">
              ...
              <div class="episode-text">
                Đang xem :
                {{
                  movie.currentPage.includes("Tập")
        ...
        <v-card class="movie-card-vertical" flat>
          ...
          <div class="card-content">
            ...
            <div class="actions">
              ...
              <v-btn variant="tonal" @click="handleFavorite(movie)">
                Bỏ theo dõi
              </v-btn>
            </div>
          </div>
        ...
        (dat) => {
          if (dat.data.status === "success") {
            const newMovies = dat.data.data.map((m) => ({
              ...
              isFavorite: true,
              notification:
                m.notification === true || m.notification === "true",
            }));
...
.recent-card:hover .recent-poster-wrap {
  border-color: rgba(255, 183, 0, 0.35);
  box-shadow: 0 12px 28px rgba(0, 0, 0, 0.6), 0 0 18px rgba(255, 140, 0, 0.14);
}
...
.movie-card-vertical:hover .movie-title {
  color: #ffb700;
}
...
.movie-card-vertical:hover {
  ...
  border-color: rgba(255, 183, 0, 0.3);
  box-shadow: 0 16px 36px rgba(0, 0, 0, 0.65), 0 0 22px rgba(255, 140, 0, 0.12);
}
